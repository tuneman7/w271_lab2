...
#fit <- auto.arima(co2_df$value, seasonal = TRUE)
fit <- auto.arima(co2_df$value, d = 1, D= 1, stepwise=FALSE, approximation=FALSE, seasonal = TRUE) 
summary(fit)

#reached out to Majid about this code; pulled from the book, and I'm not sure why auto doesn't seem to return parameters
auto_fit <- co2_df %>%
  model(
    arima013011 = ARIMA(value ~ pdq(0,1,3) + PDQ(0,1,1)),
    arima210011 = ARIMA(value ~ pdq(3,1,1) + PDQ(1,1,1)),
    auto = ARIMA(value, stepwise = FALSE, approx = FALSE)
  )

auto_fit %>% pivot_longer(everything(), names_to = "Model name",
                     values_to = "Orders")
...
auto_fit %>% 
  select(auto) %>% 
  gg_tsresiduals(lag=36) #doesn't run; not sure why

#checkresiduals(auto_fit$auto)

auto_fit %>% 
  select(auto) %>% 
  Box.test(auto,lag=10, fitdf=0, type="Lj")
```
...
Generate predictions for when atmospheric CO2 is expected to be at 420 ppm and 500 ppm levels for the
first and final times (consider prediction intervals as well as point estimates in your answer). Generate a
prediction for atmospheric CO2 levels in the year 2100. How confident are you that these will be accurate
predictions?
