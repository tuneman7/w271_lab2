...
Box.test(resid$.resid,lag=10, fitdf=0, type="Lj")
```
...
# suggests 2021-04-25, different from forecasted april 2023 from 1997 and was not within our interval of April 2022 - December 2100
...
predictions_2a <- forecast(fit_linear, h = 25*12)
...
# first fit model 3a
fit_3a = fit1
predictions_3a_pf <- forecast(fit_3a, h=25*12)

accuracy_arima = accuracy(predictions_3a_pf, co2_present_weekly_agg_monthly)
accuracy_arima
```

Task 5b: Train best models on present data
Seasonally adjust the weekly NOAA data, and split both seasonally-adjusted (SA) and non-seasonallyadjusted (NSA) series into training and test sets, using the last two years of observations as the test sets.
