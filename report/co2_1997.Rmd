...
The ARIMA model does not predict as accurately as the linear model.
...
autoplot(nsa_resid) /
ggAcf(nsa_resid)
Box.test(nsa_resid$.resid,lag=15, fitdf=0, type="Lj")
```
...
nsa_accuracy = accuracy(nsa_predictions$.mean, nsa_co2_present_test$value)
nsa_accuracy
```
