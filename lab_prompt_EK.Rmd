...
co2_df_3 <- co2_df_2  %>%
  mutate(two_diff_co2 = difference(diff_co2)) 

#EDA pt2 
plot2 <- co2_df_2 %>%
  ggplot + 
  aes(x=index, y=diff_co2) + 
  geom_line(color = 'steelblue') +
  labs(
    title = TeX(r'(Monthly Mean $CO_2$)'),
    x = 'Month and Year',
...
#model
co2_ARIMA <- co2_df %>%
  model(ARIMA(value ~ 1 + pdq(1:10,0:2,1:10) + PDQ(0,0,0), ic="aic", stepwise=F, greedy=F)) %>%
  report()
