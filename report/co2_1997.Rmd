...
pe420_min <- min(forecast420ppm$time_index) 
sprintf("Earliest 420ppm Point Estimate: %s", pe420_min)

pe420_max <-max(forecast420ppm$time_index)
sprintf("Latest 420ppm Point Estimate: %s", pe420_max)

pe420_interval <- max(forecast420ppm$time_index) - min(forecast420ppm$time_index) 
sprintf("Range of 420ppm Interval: %s", pe420_interval)

forecast500ppm <-forecast_df %>%
 filter(round(point_forecast,0) == 500 | round(lo_95,0) == 500 | round(high_95,0) == 500)

pe500_min <- min(forecast500ppm$time_index) 
sprintf("Earliest 500ppm Point Estimate: %s", pe500_min)

pe500_max <- max(forecast500ppm$time_index)
sprintf("Latest 500ppm Point Estimate: %s", pe500_max)

pe500_interval <- max(forecast500ppm$time_index) - min(forecast500ppm$time_index) 
sprintf("Range of 500ppm Interval: %s", pe500_interval)
...
Based on Keeling's autobiography, we know that there were differences between Keeling's data and the National Oceanic and Atmospheric Administration (NOAA) data until 1983, where they reconciled their measurements. However, despite this, we assume that the both datasets behave similarly. 
...
Our histogram of has no values lower than 365, or higher than 421, and that these values are not normally distributed.
...
Within these accuracy measurements displayed we will choose to talk about RMSE for the remainder of this analysis. The RMSE, or Root Mean Squared Error is the standard deviation of the observed residauls and is used to measure the overall spread of residuals, and in our context will be used to assess the accuracy of our model fits. The RMSE of the `trend model` is `13.7` whereas the RMSE of the `arima fit` is `8.26`, concluding that the `arima fit` is the more accurate model of the two. Which is loosely true, though `arima fit` did not have confidence bounds for forecasted date of observed 420ppm, it still has a point estimate that is closer than that of the `trend model`.


## ARIMA modeling

To better understand our data and predictive modeling for atmospheric $CO_2$ and to further emulate the analysis done in the 1997 report, we will move forward with using monthly data for this time series analysis. Another reason for why we have chose this method of using monthly data, is because the predictability using monthly data is theoretically and historically more accurate, due to monthly data;s ability flatten variations and pick up seasonality more reliably, thus leading to forecasts that are more stable.
...
Looking at the SA differenced ACF and PACF, the significant spike at lag=1 in the ACF suggests a non-seasonal MA(1) component while the PACF yields no new information. Let's fit a couple of models to compare, one being an $ARIMA(1,1,1)$ and one that is automatically computed through the `ARIMA` func call while adjusting to assure the estimated model retains no seasonal components.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
sa_our_model = Arima(sa_co2_present_train$season_adjust,order=c(1,1,1), include.drift=TRUE)
sa_our_model

sa_arima_fit = sa_co2_present_train %>% model(ARIMA(season_adjust~PDQ(0,0,0)))
report(sa_arima_fit)
sa_our_model = Arima(sa_co2_present_train$season_adjust,order=c(0,1,1), include.drift=TRUE)
```
Assessing the summaries from the model we chose to fit $ARIMA(1,1,1)$ with drift and without a seasonal-ARIMA process, we get an $AIC = 118$ and $BIC = 132$. Furthermore, the algorithmically chosen model gave an $ARIMA(0,1,1)$ model with $AIC = 116$ and $BIC = 126$. In either models, we observe that the automatic ARIMA call yields lower AIC and BIC values, which leads us to choose $ARIMA(0,1,1)$ for the remainder of our analysis.
...
```{r Resid TS and resid ACF1, echo=FALSE, message=FALSE, warning=FALSE}
sa_resid = residuals(sa_our_model)
...
```
From observing the residual plot, the data loosely resembles that of white noise, while the ACF of the residuals suggests that the data is stationary, with a small lag spike at lag=24 and the rest of the lags within the 95% bounds. Assessing the results from the Ljung-Box test, we have a p-value that is much larger than the 0.05 critical value leading us to fail to reject the null hypothesis and conclude that there is no serial correlation within our time series. As a note, it is interesting to observe the ACF in this case with lag=24, which indicates a seasonal MA(1) process remains, however, since we are to fit an ARIMA model to our seasonally adjusted time series, we will stay true to our subjugated methods.
...
nsa_acf <- ggAcf(nsa_co2_present_train$value, lag.max=(50)) + labs(title="Non seasonally adjusted ACF")
nsa_pacf <- ggPacf(nsa_co2_present_train$value, lag.max=(50)) + labs(title="Non seasonally adjusted PACF")

nsa_acf / nsa_pacf
```

According to the PACF plot of NSA data, we observe a spike at lag=1 with a slight spike at lag=2, this indicates an AR(1) process is needed for this data. Furthermore, the ACF of this data indicates a slow decay of spikes. Next let's test for the number of differences needed for NSA data.
...
From these results we will continue with using $ARIMA(0,1,1)$ for our seasonally adjusted time series, and $ARIMA(1,1,2)(0,1,1)[12]$ for our non-seasonally adjusted time series.
...
data_and_arima_prediction_overlay1
```
With the NSA data, we observe that the predicted values are very similar to the actual values, nearly 100% following the pattern that is observed in the actual data from Jan 2020 to May 2022. Conversely, with the predicted values of the model fitted to the SA data, we observe that the model does not perform as well and is not as accurate. It seems as though the predicted values are about a constant amount higher the actual values of $CO_2$

And now, we will fit a polynomial time trend model to the seasonally adjusted series. Here are the cross validation metrics of the polynomial time trend model:
```{r, message=FALSE, echo=FALSE, warning=FALSE, warn = FALSE}
sa_future_d1 <- select(sa_future_d,-old_model)
sa_co2_present_train2 <- select(sa_co2_present_train, -.model)
sa_fit_quadratic <- sa_co2_present_train2 %>%
  model(lm=TSLM(season_adjust ~ trend() + I(trend()^2)))
sa_fit_quadratic %>% report()

sa_quad_preds <- sa_fit_quadratic %>% forecast(new_data=sa_future_d1)
sa_quad_acc = accuracy(sa_quad_preds, sa_co2_present_test)
sa_quad_acc
```
Observing the RMSE displayed above, we can see that the $RMSE=0.459$ is less than the RMSE for the ARIMA model fit on the SA data ($RMSE=0.54$), based on this observed metric we can conclude that the polynomial time trend model is the more accurate model of the two. And below are some descriptive analyses plots of the polynomial time trend model similar to those above, analysis of which shows an even more clear picture of predictability:

```{r, echo=FALSE, warning=FALSE, message=FALSE}
sa_co2_present_test1 <- select(sa_co2_present_test1,-old_model)
sa_poly_series <- autoplot(my_sa_copy)  +
    labs(
      title = "Actual Time Series",
      subtitle = "Season adjusted data",
      x = 'Month and Year',
...
sa_poly_preds <-
   sa_quad_preds %>%
    autoplot()  +
    labs(
      title = "Polynomial Time Trend Predictions",
      subtitle = "Season adjusted data",
      x = 'Month and Year',
      y = TeX(r'($CO_2$ parts per million)')
    )
sa_poly_overlay <- 
   sa_quad_preds %>%
    autoplot(sa_co2_present_test1)  +
    labs(
      title = "Series, with Polynomial Prediction Overlay",
      subtitle = "Season adjusted data",
      x = 'Month and Year',
      y = TeX(r'($CO_2$ parts per million)')
    )
( sa_poly_series | sa_poly_preds ) / 
sa_poly_overlay

```

Here we observe the forecasted values as being very similar to that of the actual values within the test set, where nearly all of the actual predicted CO2 values are within the 80% threshold, and only a small subset of data in 2022 being outside of the 80% confidence range. We can conclude with these metrics and analyses that the polynomial time trend model is the better performant model and is more accurate model than the fitted ARIMA model.
...
It is interesting to note here that similar to the analysis done in the 1997 report, we have also estimate confidence intervals that are very wide. Based on this fact and combined with how this Keeling curve behaves, we conclude that we are not entirely confident our forecasts are accurate.
